...
from ml_utils import parse_tokens
...
def preprocess_dataset(file_bytes: bytes):
    ...
    files = {"file": ("dataset.parquet", file_bytes, "application/octet-stream")}
...
def show_learning_curves(model_id: str, cv: int, scoring: str):
    """
    Делает запрос к /plot_learning_curve, получает URL графиков и отображает их.
    """
    try:
...
def page_eda():
    ...
    if analysis_type == "Агрегированная информация":
        ...
        agg_info = df[numeric_columns].agg(["mean", "median", "std", "max", "min"])
        st.dataframe(
            agg_info.T.rename(
                columns={
                    "mean": "Среднее",
    ...
    # Облако слов
        ...
        if text_column == 'Text':
            all_tokens = [token for tokens in df['parsed_tokens'] for token in tokens]
        elif text_column == 'Tags':
            all_tokens = [token for tokens in df['parsed_tags'] for token in tokens]
...
def page_train_model():
    ...
    with st.expander("Параметры TfidfVectorizer"):
        max_features = st.slider("max_features", min_value=500, max_value=5000,
                                 value=2500, step=100)
        max_df = st.slider("max_df", min_value=0.0, max_value=1.0, value=0.9, step=0.01)
    ...
    with st.expander("Параметры классификатора"):
        classifier_type = st.selectbox("Тип классификатора", ["LogisticRegression", "SVC"])
        if classifier_type == "LogisticRegression":
            C = st.number_input("C (регуляризация)", min_value=0.0001, value=1.0)
    ...
    model_id = st.text_input("ID новой модели (любое уникальное название)", value="my_new_model")
    ...
    if st.button("Обучить модель"):
        ...
        with st.spinner("Обучение модели..."):
            try:
                ...
                st.error("Модель обучалась слишком долго (больше 10 секунд) и была прервана")
...
def page_model_info():
    ...
    if response.ok:
        ...
        if not models:
            ...
            selected_model_id = st.selectbox("Выберите модель (ID) для set", model_ids)
            if st.button("Активировать модель"):
                ...
                if req_set.ok:
                    ...
                    logger.info(f"Установлена активная модель: {selected_model_id}")
                else:
            ...
            if "active_model_id" in st.session_state:
                ...
                cv = st.number_input("Количество фолдов (cv)", min_value=2, max_value=10, value=5)
                scoring = st.selectbox("Метрика (scoring)", ["f1_macro", "f1_weighted", "roc_auc"])

                if st.button("Показать кривые обучения"):
                    with st.spinner("Генерация и загрузка кривых обучения..."):
                        show_learning_curves(st.session_state['active_model_id'], cv, scoring)
