...
# Настраиваем ротацию: maxBytes=1MB, 5 бэкапов
...
def create_pipeline(config: PipelineConfig) -> Pipeline:
    ...
    if config.classifier.type == "LogisticRegression":
        ...
        raise ValueError(f"Неподдерживаемый тип классификатора: {config.classifier.type}")

    pipeline = Pipeline([('tfidf', tfidf), ('clf', OneVsRestClassifier(classifier))])
    return pipeline
...
def _train_model_func(
    ...
    Результат возвращается через return_dict (используем multiprocessing.Manager().dict())
    """
    try:
...
@app.post("/preprocess", response_model=PreprocessResponse,
          summary='Preprocess', description="Предобработка исходного датафрейма")
async def preprocess(file: UploadFile):
    ...
    required_columns = ['author', 'publication_date', 'hubs', 'comments', 'views', 'url',
                        ...
                        'tags_tokens', 'title_tokens', 'rating_new', 'text_tokens', 'text_pos_tags',
                        'rating_level']
    missing_cols = set(required_columns) - set(df.columns)
    if missing_cols:
        logger.error(f"Не хватает столбцов в /preprocess: {missing_cols}")
        raise HTTPException(status_code=400, detail=f"Нет столбцов: {missing_cols}")
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Ошибка при предобработке данных: {str(e)}")
...
async def fit(config: str):
    ...
    try:
        ...
        raise HTTPException(status_code=400, detail=f"Ошибка в конфигурации модели: {e}")
    ...
    if p.is_alive():
        logger.warning(f"Обучение модели '{model_id}' заняло больше 10 секунд и было прервано")
        p.terminate()
        p.join()
        raise HTTPException(
            status_code=408,
            detail=f"Обучение модели '{model_id}' заняло больше 10 секунд и было прервано"
        )
    ...
    logger.info(f"Модели '{model_id_rating}' и '{model_id_hubs}' успешно обучены и сохранены.")
...
async def predict(config: str):
    ...
    try:
        ...
        raise HTTPException(status_code=400, detail=f"Ошибка в конфигурации предсказания: {e}")
    ...
    if model_id_rating not in models:
        ...
        raise HTTPException(status_code=404,
                            detail=f"Модель рейтинга'{model_id_rating}' не найдена")

    if model_id_hubs not in models:
        logger.error(f"Модель '{model_id_hubs}' не найдена")
        raise HTTPException(status_code=404,
                            detail=f"Модель хабов'{model_id_hubs}' не найдена")
    ...
    mlb = data_stor['mlb']
    inverse_rating_mapping = data_stor['inverse_rating_mapping']

    if any(param is None for param in [selector, index_to_label, mlb, inverse_rating_mapping]):
        logger.error("Вспомогательные параметры для предсказания отсутствуют в /predict")
        raise HTTPException(status_code=500,
                            detail="Вспомогательные параметры для предсказания отсутствуют.")
    try:
        # Предикт
        result_df = mu.predict_func(df=df, selector=selector,
                                    index_to_label=index_to_label, mlb=mlb,
                                    inverse_rating_mapping=inverse_rating_mapping,
                                    model_hubs=model_hubs, model_rating=model_rating)
        logger.info("Предсказание успешно")
    except Exception as e:
        logger.error(f"Ошибка при предсказании: {e}")
        raise HTTPException(status_code=400, detail=f"Ошибка при выполнении предсказания: {str(e)}")

    # Преобразуем DataFrame в список словарей (для корректной сериализации ответа в JSON)
...
@app.post("/set", summary="Set active model",
          description="Устанавливает заданную модель как текущую для предикта.")
async def set_model(model_id: str):
    ...
    if rating_key not in models:
        ...
        raise HTTPException(status_code=404, detail=f"Модель рейтинга '{rating_key}' не найдена.")

    if hubs_key not in models:
        logger.error(f"{hubs_key} не найден в /set")
        raise HTTPException(status_code=404, detail=f"Модель хабов '{hubs_key}' не найдена.")
...
@app.post("/plot_learning_curve", summary="Plot Learning Curves",
          description="Генерирует и возвращает URL-адреса двух графиков кривых обучения (rating и hubs).")
async def plt_curve(
    model_id: str,
    cv: int = 3,
